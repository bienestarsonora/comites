window.COMITES_BUILD = 'v20-borrado-archivos';
'use strict';
  ...
  const coreCard = (doc,label,icon) => doc
    ? `<article class="core-existing-card ready">
        ...
        <div class="core-existing-actions">
          ...
          ${isAdmin() ? `<button type="button" class="core-existing-delete" data-delete-committee-file="${esc(doc.id)}" data-file-label="${esc(label)}" title="Eliminar ${esc(label)}"><i class="fa-solid fa-trash"></i> Eliminar</button>` : ''}
        </div>
      </article>`
          ...
          ${extras.map(doc => `
            <div class="existing-evidence-item">
              ...
              <div class="existing-evidence-actions">
                ...
                ${isAdmin() ? `<button type="button" class="evidence-delete-btn" data-delete-committee-file="${esc(doc.id)}" data-file-label="${esc(fileName(doc))}" title="Eliminar archivo"><i class="fa-solid fa-trash"></i></button>` : ''}
              </div>
            </div>`).join('')}
        </div>
      </div>`
