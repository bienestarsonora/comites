window.COMITES_BUILD = 'v27-indice-100';
'use strict';
...
function committeeStrengthScore(committee) {
  ...
  const trainingsFor = publicTrainings.filter(
    t => String(t.committee_id||'') === id && ['Realizada','Concluida'].includes(t.status)
  );
  ...
  // Acta = 25; Lista de asistencia = 20.
  const constitucion = (hasActa ? 25 : 0) + (hasAttendance ? 20 : 0);

  // Capacitación: 35 puntos cuando existe al menos una capacitación concluida/realizada.
  const capacitacion = trainingsFor.length > 0 ? 35 : 0;

  // Seguimiento: hasta 20 puntos.
  // Se considera seguimiento cuando existen acciones, gestiones o compromisos documentados.
  let seguimiento = 0;
  const hasFollowupRecord = eventsFor.length > 0 || managementsFor.length > 0 || commitmentsFor.length > 0;
  const hasAdvancedFollowup =
    commitmentsFor.some(c => ['Cumplido','En proceso'].includes(c.status)) ||
    managementsFor.some(r => ['En gestión','Programada','En ejecución','Concluida'].includes(r.status));

  if (hasAdvancedFollowup) seguimiento = 20;
  else if (hasFollowupRecord) seguimiento = 10;

  const total = constitucion + capacitacion + seguimiento;
  return { constitucion, capacitacion, seguimiento, total, max:100 };
}
...
async function showDetail(id) {
  ...
  $('#detailContent').innerHTML = `
    ...
    <section class="strength-card"><div><span>Índice de Fortalecimiento del Comité</span><strong>${score.total}<small>/100</small></strong><b>${scoreLabel}</b></div><div class="strength-bars">${[['Constitución',score.constitucion,45],['Capacitación',score.capacitacion,35],['Seguimiento',score.seguimiento,20]].map(([label,value,max])=>`<label><span>${label}<b>${value}/${max}</b></span><i><em style="width:${Math.round(value/max*100)}%"></em></i></label>`).join('')}</div></section>
...
async function saveCommitteeAttachedFiles(committeeId) {
  ...
  // La visibilidad de archivos YA CARGADOS se administra exclusivamente
  // con los botones Público / No público del expediente.
  // Guardar el comité jamás vuelve a escribir esa visibilidad.
  ...
  // Los checkboxes inferiores sólo aplican si se selecciona un ARCHIVO NUEVO
  // para cargar o reemplazar.
  if (acta) {
    const r=await uploadCommitteeDocumentFile(
      committeeId,acta,'Acta constitutiva','Acta constitutiva',actaPublic,true
    );
  ...
  if (list) {
    const r=await uploadCommitteeDocumentFile(
      committeeId,list,'Lista de asistencia','Lista de asistencia',listPublic,true
    );
    if(!r.ok) errors.push(r.error);
  }

  const evidencePublic=$('#committeeEvidencePublic')?.checked ?? true;
  for (const file of [...($('#committeeEvidenceFiles')?.files||[])]) {
    const isImage=String(file.type||'').startsWith('image/');
    const r=await uploadCommitteeDocumentFile(
      committeeId,
      file,
      isImage?'Fotografía':'Evidencia',
      isImage?'Evidencia fotográfica':'Evidencia',
      evidencePublic,
      false
    );
    if(!r.ok) errors.push(r.error);
