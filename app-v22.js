window.COMITES_BUILD = 'v22-visibilidad-documentos';
'use strict';
...
function renderCommitteeCoreDocsStatus(committeeId = '') {
  ...
  wrap.innerHTML = `
    <div class="core-existing-title">
      ...
      <span>Puedes abrir, cambiar de Público a No público (o viceversa) y eliminar cualquier archivo ya cargado.</span>
    </div>
...
async function toggleDocumentVisibility(documentId) {
  ...
  if (error) {
    console.error(error);
    toast('No se pudo actualizar la visibilidad del archivo.', 'error');
    return;
  }
  ...
  toast(nextPublic ? 'Archivo marcado como público.' : 'Archivo marcado como no público.');
  await Promise.all([loadPublicData(), loadAdminData()]);
